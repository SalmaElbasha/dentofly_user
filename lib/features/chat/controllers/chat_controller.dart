...
class ChatController extends ChangeNotifier {
  ...
  Future<http.StreamedResponse> sendMessage(MessageBody messageBody, {int? userType}) async {
    ...
    String type = userType != null
        ? (userType == 0 ? 'delivery-man' : 'seller')
    ...
    if (response.statusCode == 200) {
      // Fetch updated messages if successful
      getMessageList(Get.context!, messageBody.id, 1, reload: false, userType: userType);
    } else {
      // Optionally log or handle error response
      final responseBody = await response.stream.bytesToString();
      debugPrint('Send message failed: ${response.statusCode}, body: $responseBody');
    }

    // Reset files and loading
    _pickedImageFiles = [];
    pickedImageFileStored = [];
    objFile = [];
    _isLoading = false;
