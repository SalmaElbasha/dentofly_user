...
class _TopSellerProductScreenState extends State<TopSellerProductScreen> with TickerProviderStateMixin {
  ...
  void _load() async {
    await Provider.of<ShopController>(Get.context!, listen: false)
        .getClearanceShopProductList('clearance_sale', '1', widget.sellerId.toString(), context);
    await Provider.of<SellerProductController>(context, listen: false)
        .getSellerProductList(widget.sellerId.toString(), 1, "", context: context);
    await Provider.of<ShopController>(Get.context!, listen: false)
  ...
  void initState() {
    ...
    _tabController = TabController(length: 2, initialIndex: widget.fromMore ? 1 : 0, vsync: this);
            ...
            SliverToBoxAdapter(
              child:  Padding(
                padding: const EdgeInsets.fromLTRB(
                    ...
                    Dimensions.paddingSizeSmall,
                    0),
                child: ShopProductViewList(sellerId: widget.sellerId!),
              ),
            ),
          ]);
        }),
