...
class ShopProductViewList extends StatefulWidget {
  ...
  final int sellerId;
  const ShopProductViewList({super.key,  this.scrollController, required this.sellerId});
...
class _ShopProductViewListState extends State<ShopProductViewList> {
  void _load() async{
    await Provider.of<SellerProductController>(context, listen: false).getSellerProductList(widget.sellerId.toString(), 1, "",context: context);
  }
  @override
  void initState() {
    super.initState();
    ...
    return Consumer<SellerProductController>(
      builder: (context, productController, _) {
        ...
        PaginatedListView(scrollController: widget.scrollController,
            onPaginate: (offset) async=> await productController.getSellerProductList(widget.sellerId.toString(), offset!, "", reload: false,context: context),
