...
class ClearanceProductFilterDialog extends StatefulWidget {
  ...
  List<AuthorModel>? publishingHouse;
  final void Function() applyFilter;

  ClearanceProductFilterDialog({super.key, this.sellerId,  this.fromShop = true, this.authorList, this.publishingHouse, required this.applyFilter});
...
class ClearanceProductFilterDialogState extends State<ClearanceProductFilterDialog> {
  ...
  void initState() {
    ...
    if(widget.authorList!.isNotEmpty) {
      for (int i =0; i< widget.authorList!.length; i++) {
        authors.add(i);
      }
    }

    if(widget.publishingHouse!.isNotEmpty) {
      for (int i=0; i < widget.publishingHouse!.length; i++) {
  ...
  Widget build(BuildContext context) {
    ...
    return Dismissible(
      ...
      child: Consumer<SearchProductController>(builder: (context, searchProvider, child) {
        if(widget.fromShop){
          widget.authorList = searchProvider.sellerAuthorsList;
          widget.publishingHouse = searchProvider.sellerPublishingHouseList;
        }else{
          widget.authorList = searchProvider.authorsList;
          widget.publishingHouse = searchProvider.publishingHouseList;
        }
                          ...
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                            ...
                            (categoryProvider.selectedCategoryIds.isNotEmpty || brandProvider.selectedBrandIds.isNotEmpty
                              ...
                              onTap: () async {
                                ...
                                await categoryProvider.resetChecked(widget.fromShop ? widget.sellerId! : null, widget.fromShop,context);
                          ...
                          child: SingleChildScrollView(
                            ...
                            child: Column( crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min,
                              children: [
                                ...
                                //Author
                                if((widget.authorList!.isNotEmpty) && (searchProvider.productTypeIndex == 0 || searchProvider.productTypeIndex == 2))...[
                                  const SizedBox(height: Dimensions.paddingSizeSmall),
                                  ...
                                  Autocomplete<int> (
                                    optionsBuilder: (TextEditingValue value) {
                                      if (value.text.isEmpty) {
                                        ...
                                        return authors.where((author) => widget.authorList![author].name!.toLowerCase().contains(value.text.toLowerCase()));
                                      }
                                    },
                                    ...
                                    displayStringForOption: (value) =>  widget.authorList![value].name!,
                                    onSelected: (int value) {
                                      searchProvider.checkedToggleAuthors(value, widget.fromShop);
                                    },
                                  ),

                                  if(widget.authorList != null && widget.authorList!.isNotEmpty)
                                    ConstrainedBox(
                                      constraints: const BoxConstraints(
                                        minHeight: 40.0,
                                        maxHeight: 350.0,
                                      ...
                                      child: SizedBox(
                                        child: ListView.builder(
                                            ...
                                            itemBuilder: (context, index){
                                              return Column(children: [
                                                CategoryFilterItem(title: widget.authorList![index].name,
                                                  checked: widget.authorList![index].isChecked!,
                                                  onTap: () {
                                ...
                                //Publishing House
                                if((widget.publishingHouse != null && widget.publishingHouse!.isNotEmpty) && (searchProvider.productTypeIndex == 0 || searchProvider.productTypeIndex == 2))...[
                                  const SizedBox(height: Dimensions.paddingSizeSmall),
                                  ...
                                  Autocomplete<int> (
                                    optionsBuilder: (TextEditingValue value) {
                                      if (value.text.isEmpty) {
                                        ...
                                        return publishingHouses.where((author) => widget.publishingHouse![author].name!.toLowerCase().contains(value.text.toLowerCase()));
                                      }
                                    },
                                    fieldViewBuilder: (context, controller, node, onComplete) {
                                      return Container(
                                    ...
                                    displayStringForOption: (value) =>  widget.publishingHouse![value].name!,
                                    onSelected: (int value) {
                                      searchProvider.checkedTogglePublishingHouse(value, widget.fromShop);
                                    },
                                  ),

                                  if(widget.publishingHouse != null && widget.publishingHouse!.isNotEmpty)
                                    ConstrainedBox(
                                      constraints: const BoxConstraints(
                                        minHeight: 40.0,
                                        maxHeight: 350.0,
                                      ...
                                      child: SizedBox(
                                        child: ListView.builder(
                                            itemCount: widget.publishingHouse?.length,
                                            shrinkWrap: true,
                                            itemBuilder: (context, index){
                                              return Column(children: [
                                                CategoryFilterItem(title: widget.publishingHouse![index].name,
                                                    checked: widget.publishingHouse![index].isChecked!,
