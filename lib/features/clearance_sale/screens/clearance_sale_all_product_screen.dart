...
class _ClearanceSaleAllProductScreenState extends State<ClearanceSaleAllProductScreen> {
  ...
  void initState() {
    ...
    Provider.of<ProductController>(Get.context!, listen: false).getClearanceSearchProduct(offset: 1, query: '', isNotify: false,context: context);
  }
                ...
                child: SingleChildScrollView(
                  ...
                  child: Consumer<ProductController>(builder: (context, productController, _){
                    ...
                    return (productModel != null && !productController.isSearchLoading)
                        ...
                        onPaginate: (offset) async => applyFilter(context: context, fromPagination: true, offset: offset),
...
void applyFilter({required BuildContext context, bool fromPagination = false, int? offset}) {
  ...
  // } else if (searchProvider.productTypeIndex == 2 && (searchProvider.selectedAuthorIds.isEmpty && searchProvider.publishingHouseIds.isEmpty)){
  ...
  // else{
  //
  //   productController.isSearchLoading = true;
  //
  ...
  //     );
  //   } else {
  //     productController.getClearanceSearchProduct(query: "",categoryIds: selectedCategoryId,
  //       brandIds: selectedBrandId, authorIds: selectedAuthorId, publishingIds: selectedPublishingId, offset: 1,
  //       productType:  searchProvider.productTypeIndex == 0 ? 'all' : searchProvider.productTypeIndex == 1 ? 'physical' : 'digital', offerType: 'clearance_sale'
  //     ).then((value) {
  //       if(value.response?.statusCode == 200){
  //         productController.isSearchLoading = false;
  ...
  if(fromPagination) {
    productController.getClearanceSearchProduct(query: productController.isSearchActive ? productController.searchText ?? '' : '', categoryIds: selectedCategoryId,
        brandIds: selectedBrandId, authorIds: selectedAuthorId, publishingIds: selectedPublishingId, offset: offset ?? 1,
        productType:  searchProvider.productTypeIndex == 0 ? 'all' : searchProvider.productTypeIndex == 1 ? 'physical' : 'digital', offerType: 'clearance_sale', fromPaginantion: fromPagination, context: context
    );
  } else {
    ...
    productController.getClearanceSearchProduct(query: productController.isSearchActive ? productController.searchText ?? '' : '',categoryIds: selectedCategoryId,
        brandIds: selectedBrandId, authorIds: selectedAuthorId, publishingIds: selectedPublishingId, offset: 1,
        productType:  searchProvider.productTypeIndex == 0 ? 'all' : searchProvider.productTypeIndex == 1 ? 'physical' : 'digital', offerType: 'clearance_sale', context: context
    ).then((value) {
      if(value.response?.statusCode == 200){
        productController.isFilterActive = true;
      }
...
void onClick( BuildContext context, ProductController productController, TextEditingController searchController){
  if(searchController.text.isEmpty && !productController.isSearchActive) {
    ...
    productController.getClearanceSearchProduct(offset: 1, query: productController.searchText!, context: context);
    ...
    productController.toggleSearchActive();
    productController.getClearanceSearchProduct(offset: 1, query: '', context: context);
  }
}
