...
class OrderDetailsController with ChangeNotifier {
  ...
  Future<List<OrderDetailsModel>> fetchOrderDetailsList(String orderID) async {
    List<OrderDetailsModel> orderDetailsList = [];

    ApiResponse apiResponse = await orderDetailsServiceInterface.getOrderDetails(orderID);

    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      for (var order in apiResponse.response!.data) {
        orderDetailsList.add(OrderDetailsModel.fromJson(order));
      }
    }
