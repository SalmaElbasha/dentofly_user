    ...
    scrollController?.addListener(() async {
      if(scrollController!.position.maxScrollExtent == scrollController!.position.pixels &&
        ...
        if(offset < pageSize) {
          ...
          if(productType == ProductType.featuredProduct){
            Provider.of<ProductController>(context, listen: false).getFeaturedProductList(offset.toString(),context);
          }else {
            Provider.of<ProductController>(context, listen: false).getLatestProductList(offset,context);
          }
        }else{
    ...
    return Consumer<ProductController>(
      builder: (context, prodProvider, child) {
        ...
        if(productType == ProductType.latestProduct) {
          productList = prodProvider.latestProductList;  // هنا القائمة المفلترة حسب المحافظات جاهزة للاستخدام
        }
