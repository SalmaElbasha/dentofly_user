...
class _ProfileScreen1State extends State<ProfileScreen1> {
  ...
  _updateUserAccount() async {
    ...
    dynamic? governate_id=profile.governates?.firstWhere((governateId) => governateId.name == profile.selectedGovernate).id??governateService.getGovernate();
    ...
    //     && file == null && _passwordController.text.isEmpty && _confirmPasswordController.text.isEmpty
      ...
      updateUserInfoModel.governorate_id=governate_id;
      await Provider.of<ProfileController>(context, listen: false).updateUserInfo(
        ...
        if(response.isSuccess) {
          ...
          await governateService.saveGovernate(Provider.of<ProfileController>(context, listen: false).governates?.firstWhere((governateId) => governateId.name == Provider.of<ProfileController>(context, listen: false).selectedGovernate).id??governateService.getGovernate()??"");
  ...
  void initState() {
    ...
    Future.microtask(() async {
      ...
      await profile.getGovernates();
      final names = profile.governatesNames;
      if (mounted) {
        Provider.of<ProfileController>(context, listen: false).setGovernates(names??[]);
      }
    });
  }
          ...
          builder: (context,authController,_) {
            return Consumer<ProfileController>(
              builder: (context,profile,_) {
                ...
                labelText: getTranslated('governate', context),
                ...
                value: (profile.governatesNames ?? []).contains(profile.selectedGovernate)
                ? profile.selectedGovernate
                    : null,
                items: (profile.governatesNames ?? []).map((name) {
                ...
                onChanged: (value) {
                setState(() {
                profile.selectedGovernate = value;
                });
                },
                ),
