...
class AsterThemeHomeScreen extends StatefulWidget {
  ...
  static Future<void> loadData(bool reload,BuildContext context) async {
    ...
    await shopController.getAllSellerList(reload, 1,context, type: "all");
    ...
    productController.getHomeCategoryProductList(context,reload);

    shopController.getTopSellerList(reload, 1,context, type: "top");

    brandController.getBrandList(reload);

    productController.getLatestProductList(1,context, reload: reload);

    await productController.getFeaturedProductList('1',context, reload: reload);

    featuredDealController.getFeaturedDealList(reload,context);

    productController.getLProductList('1',context, reload: reload);

    productController.getRecommendedProduct(context);

    productController.findWhatYouNeed(context);

    productController.getJustForYouProduct(context);

    shopController.getMoreStore();

    productController.getClearanceAllProductList('1',context);
    ...
    if(Provider.of<AuthController>(Get.context!, listen: false).isLoggedIn()){
      ...
      sellerProductController.getShopAgainFromRecentStore(context);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(resizeToAvoidBottomInset: false,
      body: SafeArea(child: RefreshIndicator(
          onRefresh: () async {
            await AsterThemeHomeScreen.loadData( true,context);
          },
                          ...
                          PopupMenuButton(
                            ...
                            onSelected: (dynamic value) async{
                              if(value == ProductType.newArrival){
                                Provider.of<ProductController>(context, listen: false).changeTypeOfProduct(value, types[0],context);
                              }else if(value == ProductType.topProduct){
                                Provider.of<ProductController>(context, listen: false).changeTypeOfProduct(value, types[1],context);
                              }else if(value == ProductType.bestSelling){
                                Provider.of<ProductController>(context, listen: false).changeTypeOfProduct(value, types[2],context);
                              }else if(value == ProductType.discountedProduct){
                                Provider.of<ProductController>(context, listen: false).changeTypeOfProduct(value, types[3],context);
                              }

                              Provider.of<ProductController>(context, listen: false).getLatestProductList(1,context, reload: true);
                            },
                          ) : const SizedBox(),
                        ]),
                      );
