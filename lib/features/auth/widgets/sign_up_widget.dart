...
import '../domain/repositories/GovernateRepository.dart';
import '../domain/services/GovernateService.dart';
...
class SignUpWidgetState extends State<SignUpWidget> {
  ...
  void initState() {
   ...
   Provider.of<AuthController>(context, listen: false).setCountryCode(CountryCode.fromCountryCode(Provider.of<SplashController>(context, listen: false).configModel!.countryCode!).dialCode!, notify: false);
    Provider.of<AuthController>(context, listen: false).getGovernates();
  }
          ...
          builder: (context, authProvider, _) {
            return Consumer<SplashController>(
              builder: (context, splashProvider,_) {
                ...
                isExpanded: true,
                hint: Text("Choose governate",style: textRegular.copyWith(fontSize: Dimensions.fontSizeDefault,color: ColorResources.black),),
                value: authProvider.selectedGovernate,
                items: authProvider.governatesNames?.map((name) {
                return DropdownMenuItem<String>(
                value: name,
                child: Text(name,style: textRegular.copyWith(fontSize: Dimensions.fontSizeDefault, color: Colors.black)),
                );
                }).toList(),
                onChanged: (value) {
                  authProvider.selectedGovernate = value;
                  setState(() {
                      ...
                      child: CustomButton(
                        ...
                        onTap: authProvider.isAcceptTerms ?  () async {
                          ...
                          if(signUpFormKey.currentState?.validate() ?? false) {
                            ...
                            register.governateId=authProvider.governates?.firstWhere((governateId) => governateId.name == authProvider.selectedGovernate).id;
                            authProvider.registration(register, route, config!);
                            await governateService.saveGovernate(authProvider.governates?.firstWhere((governateId) => governateId.name == authProvider.selectedGovernate).id??governateService.getGovernate()??"");
                            String? saved = governateService.getGovernate();
                            print(saved);
                          }
