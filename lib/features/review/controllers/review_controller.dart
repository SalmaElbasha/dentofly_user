...
class ReviewController extends ChangeNotifier {
  ...
  final Map<String, ReviewModel?> _orderWiseReviews = {};

// Helper method to generate unique key for each product-order pair
  String _reviewKey(String productId, String orderId) => '${productId}_$orderId';

// Public getter to retrieve a specific order-wise review
  ReviewModel? getOrderWiseReviewData(String productId, String orderId) {
    return _orderWiseReviews[_reviewKey(productId, orderId)];
  }
  ...
  Future<ResponseModel> submitReview(ReviewBody reviewBody, List<File> files, bool update) async {
    ...
    if (response.statusCode == 200) {
      ...
      responseModel = ResponseModel('${response.statusCode} ${response.toString()}', false);
    }
    _isLoading = false;
    notifyListeners();
  ...
  Future<void> getOrderWiseReview(String productId, String orderId, {bool showLoading = false}) async {
    ...
    try {
      ...
      final key = _reviewKey(productId, orderId);

      if (reviewResponse.response != null &&
          reviewResponse.response!.statusCode == 200 &&
          reviewResponse.response!.data != null) {

        final dynamic data = reviewResponse.response!.data;

        if (data is Map<String, dynamic>) {
          _orderWiseReviews[key] = ReviewModel.fromJson(data);
        } else if (data is List) {
          _orderWiseReviews[key] = null;
        } else {
          _orderWiseReviews[key] = null;
        }
      } else {
      ...
      debugPrint('Error in getOrderWiseReview: $e');
    }
