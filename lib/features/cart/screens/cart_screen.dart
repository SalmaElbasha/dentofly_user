...
class CartScreenState extends State<CartScreen> {
  Future<void> _loadData() async{
    ...
    Provider.of<SplashController>(Get.context!, listen: false).configModel!.shippingMethod = 'sellerwise_shipping';
        ...
        return Consumer<ShippingController>(
          builder: (context, shippingController,_) {
            return Consumer<CartController>(builder: (context, cart, child) {
              double amount = 0.0;
              double shippingAmount = Provider.of<AddressController>(context, listen: false).shippingCost??0;
              double discount = 0.0;
              double tax = 0;
              int totalQuantity = 0;
              int totalPhysical = 0;
              ...
              for(int j = 0; j< cartList.length; j++){
                if(cartList[j].isChecked!) {
                  shippingAmount += cartList[j].shippingCost ?? 0;
                                      ...
                                      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                        ...
                                        Row(
                                          children: [
                                            ...
                                            // User Location
                                            if (Provider.of<CheckoutController>(context, listen: false).userLat != null && Provider.of<CheckoutController>(context, listen: false).userLng != null)
                                              Text(
                                                '(${Provider.of<CheckoutController>(context, listen: false).userLat!.toStringAsFixed(2)}, ${Provider.of<CheckoutController>(context, listen: false).userLng!.toStringAsFixed(2)})',
                                                style: textRegular,
                                              )
                                            else
                                              Text(
